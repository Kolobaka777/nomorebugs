...
export default function BaselineSurvey({ onComplete }: BaselineSurveyProps) {
  ...
  const skills = [
    { key: 'html_structure', label: 'HTML структура', icon: '🏗️', desc: 'Понимание тегов, семантики, атрибутов' },
    { key: 'css_reading', label: 'Чтение CSS', icon: '🎨', desc: 'Чтение и понимание стилей' },
    { key: 'devtools', label: 'DevTools', icon: '🔧', desc: 'Работа с инструментами разработчика' },
    { key: 'console_errors', label: 'Ошибки консоли', icon: '⚠️', desc: 'Чтение и понимание ошибок JS' },
    { key: 'bug_report_quality', label: 'Баг-репорты', icon: '🐛', desc: 'Умение описывать дефекты' },
  ];
      ...
      <div className="w-full max-w-2xl relative z-10 fade-in">
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div key={skill.key}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-pixel/40 text-xs font-sans">{skill.desc}</p>
                  </div>
                </div>
                ...
                {/* Selected label */}
                <p className="text-pixel/30 text-xs font-sans mt-1.5 text-right">
            ...
            >
              {loading
                ? <span className="pixel-pulse">🐌 сохраняем...</span>
                : '🐞 Начать обучение →'
              }
            </button>
          </form>
