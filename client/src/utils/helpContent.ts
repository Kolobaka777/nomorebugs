...
export interface HowToItem {
  id: string;
  icon: IconName;
  // One form, not two. This used to carry an imperative `title` for the
  // Help page's capability list alongside a `question` for the mascot's
  // chat — and the two moods ended up side by side anyway, because Помощь
  // shows this list and the FAQ as neighbouring tabs: «Собирай курсы» in
  // one, «Как создать курс?» in the other. Everywhere the app offers a
  // thing to click, it asks a question; a button that gives an order reads
  // as the app telling you what to do, and in the chat it made the person
  // order the frog about instead of asking it something.
  question: string;
  body: string;
}
