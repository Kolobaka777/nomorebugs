...
beforeAll(async () => {
  ...
  await request(server).post('/api/auth/register').send({ email: 'other-kb-tester@test.local', password: 'otherpass123', name: 'Other Tester' });
  otherTesterToken = await loginAs(request, server, 'other-kb-tester@test.local', 'otherpass123');
});
...
describe('bug-example proposals', () => {
  ...
  it('a tester proposing a bug example gets a pending, unpublished row, invisible to an unrelated tester', async () => {
    ...
    const otherList = await request(server).get('/api/bug-examples').set('Authorization', `Bearer ${otherTesterToken}`);
    expect(otherList.body.find(e => e.id === proposalId)).toBeUndefined();

    const ownList = await request(server).get('/api/bug-examples').set('Authorization', `Bearer ${testerToken}`);
    expect(ownList.body.find(e => e.id === proposalId)).toBeTruthy();

    const leadList = await request(server).get('/api/bug-examples').set('Authorization', `Bearer ${leadToken}`);
  ...
  it('a lead approving it publishes the example for everyone', async () => {
    const res = await request(server).patch(`/api/bug-examples/${proposalId}/approve`).set('Authorization', `Bearer ${leadToken}`);
    expect(res.status).toBe(200);
  ...
  it('a lead declining a proposal soft-deletes it, stamps proposal_status=rejected, and notifies the author', async () => {
    ...
    const del = await request(server).delete(`/api/bug-examples/${id}`).set('Authorization', `Bearer ${leadToken}`);
  ...
  it('a tester cannot approve their own or anyone else\'s proposal', async () => {
    ...
    const res = await request(server).patch(`/api/bug-examples/${created.body.id}/approve`).set('Authorization', `Bearer ${testerToken}`);
  ...
  it('approving something that is not a pending proposal is rejected', async () => {
    ...
    const res = await request(server).patch(`/api/bug-examples/${created.body.id}/approve`).set('Authorization', `Bearer ${leadToken}`);
  ...
  it('seeded bug examples (created_by NULL) still appear in every list — a LEFT JOIN, not an inner join, on the author', async () => {
    ...
    const list = await request(server).get('/api/bug-examples').set('Authorization', `Bearer ${otherTesterToken}`);
...
describe('glossary-term proposals', () => {
  ...
  it('a tester proposing a term gets a pending, unpublished row; approving it publishes for everyone', async () => {
    ...
    const otherList = await request(server).get('/api/glossary').set('Authorization', `Bearer ${otherTesterToken}`);
    expect(otherList.body.find(g => g.id === proposalId)).toBeUndefined();

    notifyUserMock.mockClear();
    const approve = await request(server).patch(`/api/glossary/${proposalId}/approve`).set('Authorization', `Bearer ${leadToken}`);
    expect(approve.status).toBe(200);

    const nowVisible = await request(server).get('/api/glossary').set('Authorization', `Bearer ${otherTesterToken}`);
  ...
  it('seeded glossary terms (created_by NULL) still appear in every list', async () => {
    ...
    expect(seeded).toBeTruthy();
    const list = await request(server).get('/api/glossary').set('Authorization', `Bearer ${otherTesterToken}`);
  ...
  it('declining a term proposal soft-deletes it and stamps proposal_status=rejected', async () => {
    ...
    const id = created.body.id;
    const del = await request(server).delete(`/api/glossary/${id}`).set('Authorization', `Bearer ${leadToken}`);
    expect(del.status).toBe(200);
...
describe('questions on the suggestions board', () => {
  ...
  it('a tester can post a question; it is visible to everyone, unanswered', async () => {
    ...
    const list = await request(server).get('/api/suggestions').set('Authorization', `Bearer ${otherTesterToken}`);
  ...
  it('answering a non-question suggestion is rejected', async () => {
    const idea = await request(server)
      .post('/api/suggestions')
      .set('Authorization', `Bearer ${testerToken}`)
