...
describe('course deadlines — per-user override', () => {
  it('an override takes precedence over the course default for that user only', async () => {
    ...
    const asTester = await request(server).get(`/api/custom-courses/${courseId}`).set('Authorization', `Bearer ${testerToken}`);
    ...
    const asLead = await request(server).get(`/api/custom-courses/${courseId}`).set('Authorization', `Bearer ${leadToken}`);
