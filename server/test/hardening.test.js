...
describe('login timing side-channel fix', () => {
  it('still pays the bcrypt cost for a nonexistent email (does not short-circuit)', async () => {
    ...
    spy.mockClear();
    await request(server).post('/api/auth/login').send({ email: 'nobody-at-all@test.local', password: 'whatever123' });
    expect(spy).toHaveBeenCalled();
    spy.mockRestore();
  });

  it('a nonexistent email still returns the same 401/"Invalid credentials" shape as a wrong password', async () => {
    const nonexistent = await request(server).post('/api/auth/login').send({ email: 'nobody-at-all@test.local', password: 'whatever123' });
    const wrongPassword = await request(server).post('/api/auth/login').send({ email: 'admin@test.local', password: 'wrongpassword123' });
  ...
  it('a correct login still succeeds (no regression from the constant-time change)', async () => {
    const res = await request(server).post('/api/auth/login').send({ email: 'admin@test.local', password: 'adminpass123' });
