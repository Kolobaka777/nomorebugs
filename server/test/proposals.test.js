...
beforeAll(async () => {
  ...
  await request(server).post('/api/auth/register').send({ email: 'other-proposal-tester@test.local', password: 'otherpass123', name: 'Other Tester' });
  otherTesterToken = await loginAs(request, server, 'other-proposal-tester@test.local', 'otherpass123');
});
...
describe('course proposals', () => {
  ...
  it('a tester submitting a valid course gets a pending, unpublished row', async () => {
    const res = await request(server).post('/api/custom-courses').set('Authorization', `Bearer ${testerToken}`).send(validCourseBody());
  ...
  it('is invisible to an unrelated tester, visible to its own author, and visible in the lead\'s review queue', async () => {
    const otherList = await request(server).get('/api/custom-courses').set('Authorization', `Bearer ${otherTesterToken}`);
    expect(otherList.body.find(c => c.id === proposalId)).toBeUndefined();

    const ownList = await request(server).get('/api/custom-courses').set('Authorization', `Bearer ${testerToken}`);
    expect(ownList.body.find(c => c.id === proposalId)).toBeTruthy();

    const leadList = await request(server).get('/api/custom-courses').set('Authorization', `Bearer ${leadToken}`);
    ...
    const otherDetail = await request(server).get(`/api/custom-courses/${proposalId}`).set('Authorization', `Bearer ${otherTesterToken}`);
  ...
  it('a lead approving it publishes the course and credits the news event to the original author', async () => {
    const res = await request(server).patch(`/api/custom-courses/${proposalId}/publish`).set('Authorization', `Bearer ${leadToken}`);
    expect(res.status).toBe(200);
  ...
  it('a lead declining a proposal soft-deletes it, stamps proposal_status=rejected, and notifies the author', async () => {
    ...
    const created = await request(server).post('/api/custom-courses').set('Authorization', `Bearer ${testerToken}`).send(validCourseBody({ title: 'To be declined' }));
    declinedId = created.body.id;

    const del = await request(server).delete(`/api/custom-courses/${declinedId}`).set('Authorization', `Bearer ${leadToken}`);
  ...
  it('a tester still cannot directly edit, delete, or publish a course — proposing is a one-shot submission', async () => {
    const created = await request(server).post('/api/custom-courses').set('Authorization', `Bearer ${testerToken}`).send(validCourseBody({ title: 'No self-edit' }));
    const id = created.body.id;

    const put = await request(server).put(`/api/custom-courses/${id}`).set('Authorization', `Bearer ${testerToken}`).send(validCourseBody({ title: 'Edited by author' }));
    expect(put.status).toBe(403);

    const del = await request(server).delete(`/api/custom-courses/${id}`).set('Authorization', `Bearer ${testerToken}`);
    expect(del.status).toBe(403);
  });

  it('a lead still cannot manage another lead\'s ordinary (non-proposal) course they did not author', async () => {
    await request(server).post('/api/auth/register').send({ email: 'second-lead-proposals@test.local', password: 'leadpass123', name: 'Second Lead' });
    ...
    const secondLeadToken = await loginAs(request, server, 'second-lead-proposals@test.local', 'leadpass123');

    const ownCourse = await request(server).post('/api/custom-courses').set('Authorization', `Bearer ${secondLeadToken}`).send(validCourseBody({ title: 'Second lead\'s own course' }));
    expect(ownCourse.status).toBe(200);

    const blocked = await request(server).delete(`/api/custom-courses/${ownCourse.body.id}`).set('Authorization', `Bearer ${leadToken}`);
...
describe('guide proposals', () => {
  ...
  it('a tester proposing a guide gets a pending, unpublished row; approving it publishes and credits the author', async () => {
    ...
    const otherDetail = await request(server).get(`/api/guides/${guideId}`).set('Authorization', `Bearer ${otherTesterToken}`);
    expect(otherDetail.status).toBe(403);

    const leadList = await request(server).get('/api/guides').set('Authorization', `Bearer ${leadToken}`);
    const inList = leadList.body.find(g => g.id === guideId);
    expect(inList).toBeTruthy();
    expect(inList.proposal_status).toBe('pending');

    const approve = await request(server).patch(`/api/guides/${guideId}/approve`).set('Authorization', `Bearer ${leadToken}`);
    ...
    const nowVisible = await request(server).get(`/api/guides/${guideId}`).set('Authorization', `Bearer ${otherTesterToken}`);
  ...
  it('declining a guide proposal soft-deletes it, stamps proposal_status=rejected, and notifies the author', async () => {
    ...
    const del = await request(server).delete(`/api/guides/${id}`).set('Authorization', `Bearer ${leadToken}`);
    expect(del.status).toBe(200);
  ...
  it('rejects approving something that is not a pending proposal', async () => {
    const res = await request(server).patch(`/api/guides/${guideId}/approve`).set('Authorization', `Bearer ${leadToken}`);
    expect(res.status).toBe(400);
  });
});
...
describe('proposal counts on profile', () => {
  ...
  it('proposal counts are NOT visible to a colleague on the tester\'s public profile, but a lead still sees them', async () => {
    const colleagueView = await request(server).get(`/api/users/${fixtures.testerId}/profile`).set('Authorization', `Bearer ${otherTesterToken}`);
    ...
    const leadView = await request(server).get(`/api/users/${fixtures.testerId}/profile`).set('Authorization', `Bearer ${leadToken}`);
