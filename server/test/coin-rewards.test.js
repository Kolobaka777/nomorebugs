...
describe('coins for approved proposals', () => {
  it('pays the author when a lead approves their course, and pays it once', async () => {
    ...
    await request(app).patch(`/api/custom-courses/${proposal.body.id}/publish`).set('Authorization', `Bearer ${leadToken}`);
  ...
  it('pays for an approved guide, bug example and glossary term', async () => {
    ...
    const guide = await request(app).post('/api/guides').set('Authorization', `Bearer ${testerToken}`)
      .send({ title: 'Coin Guide', category: 'Общее', content: 'body' });
    await request(app).patch(`/api/guides/${guide.body.id}/approve`).set('Authorization', `Bearer ${leadToken}`);

    const example = await request(app).post('/api/bug-examples').set('Authorization', `Bearer ${testerToken}`)
      .send({ tag: 'UI', tag_color: '#fff', problem: 'p', bad_text: 'bad', good_text: 'good' });
    await request(app).patch(`/api/bug-examples/${example.body.id}/approve`).set('Authorization', `Bearer ${leadToken}`);

    const term = await request(app).post('/api/glossary').set('Authorization', `Bearer ${testerToken}`)
      .send({ term: 'Coin Term', definition: 'd' });
    await request(app).patch(`/api/glossary/${term.body.id}/approve`).set('Authorization', `Bearer ${leadToken}`);
  ...
  it('pays nothing when the approver is the author — otherwise the reward is self-serve for anyone holding the permission', async () => {
    ...
    const guide = await request(app).post('/api/guides').set('Authorization', `Bearer ${leadToken}`)
    ...
    const res = await request(app).patch(`/api/guides/${guide.body.id}/approve`).set('Authorization', `Bearer ${leadToken}`);
...
describe('coins for finishing a course', () => {
  async function makeCourse(lessonCount) {
    const res = await request(app).post('/api/custom-courses').set('Authorization', `Bearer ${leadToken}`)
      .send({
  ...
  it('pays once when every lesson is done', async () => {
    ...
    for (const id of lessonIds) {
      await request(app).post(`/api/custom-lessons/${id}/complete`).set('Authorization', `Bearer ${testerToken}`);
    }
    const before = coinsOf(fixtures.testerId);

    await request(app).post('/api/courses/time-track').set('Authorization', `Bearer ${testerToken}`)
  ...
  it('pays nothing when the lessons are not actually done — the request body cannot vouch for itself', async () => {
    ...
    await request(app).post(`/api/custom-lessons/${lessonIds[0]}/complete`).set('Authorization', `Bearer ${testerToken}`);
    const before = coinsOf(fixtures.testerId);

    const res = await request(app).post('/api/courses/time-track').set('Authorization', `Bearer ${testerToken}`)
      .send({ course_id: courseId, seconds_spent: 60 });
