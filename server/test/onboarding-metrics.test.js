...
describe('GET /api/tester/metrics — onboardingCourseCompleted/onboardingCourseId', () => {
  it('is false/null when no onboarding course is published yet', async () => {
    const res = await request(server).get('/api/tester/metrics').set('Authorization', `Bearer ${testerToken}`);
  ...
  it('is false, with the course id, once one is published but not finished', async () => {
    const created = await request(server).post('/api/custom-courses').set('Authorization', `Bearer ${leadToken}`).send({
    ...
    const course = await request(server).get(`/api/custom-courses/${courseId}`).set('Authorization', `Bearer ${testerToken}`);
    [lesson1Id, lesson2Id] = course.body.modules[0].lessons.map(l => l.id);

    await request(server).post(`/api/custom-lessons/${lesson1Id}/complete`).set('Authorization', `Bearer ${testerToken}`);
  ...
  it('flips true (and the id back to null) once every lesson is completed', async () => {
    await request(server).post(`/api/custom-lessons/${lesson2Id}/complete`).set('Authorization', `Bearer ${testerToken}`);
    const res = await request(server).get('/api/tester/metrics').set('Authorization', `Bearer ${testerToken}`);
  ...
  it('a second tester who never touched it still sees it as incomplete', async () => {
    await request(server).post('/api/auth/register').send({ email: 'fresh-onboarding-tester@test.local', password: 'freshpass123', name: 'Fresh Tester' });
    const freshToken = await loginAs(request, server, 'fresh-onboarding-tester@test.local', 'freshpass123');
    const res = await request(server).get('/api/tester/metrics').set('Authorization', `Bearer ${freshToken}`);
    expect(res.body.onboardingCourseCompleted).toBe(false);
    expect(res.body.onboardingCourseId).toBe(courseId);
  });

  it('does not count a draft (unpublished) onboarding course', async () => {
    await request(server).post('/api/custom-courses').set('Authorization', `Bearer ${leadToken}`).send({
