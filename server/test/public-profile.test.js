...
describe('GET /api/users/:id/profile', () => {
  it('a public profile (default) shows the full read-only view to anyone', async () => {
    const res = await request(app).get(`/api/users/${otherTesterId}/profile`).set('Authorization', `Bearer ${testerToken}`);
  ...
  it('a private profile only shows avatar/name + is_public:false to a stranger', async () => {
    await request(app).put('/api/tester/profile').set('Authorization', `Bearer ${otherTesterToken}`).send({ is_public: false }).expect(200);
  ...
  it('the owner can always see their own profile in full, even when private', async () => {
    const res = await request(app).get(`/api/users/${otherTesterId}/profile`).set('Authorization', `Bearer ${otherTesterToken}`);
    expect(res.status).toBe(200);
    expect(res.body).toHaveProperty('stats');
  });

  it('a lead can always see the full profile, even when private', async () => {
    const res = await request(app).get(`/api/users/${otherTesterId}/profile`).set('Authorization', `Bearer ${leadToken}`);
    expect(res.status).toBe(200);
    expect(res.body).toHaveProperty('stats');
  });
  ...
  it('includes presence/working-hours fields when visible', async () => {
    ...
    await request(app).put('/api/tester/profile').set('Authorization', `Bearer ${otherTesterToken}`).send({ is_public: true }).expect(200);
  ...
  it('404s for a nonexistent or archived user', async () => {
    const missing = await request(app).get('/api/users/999999/profile').set('Authorization', `Bearer ${testerToken}`);
    expect(missing.status).toBe(404);

    await request(app).post(`/api/admin/users/${otherTesterId}/archive`).set('Authorization', `Bearer ${leadToken}`).expect(200);
    const archived = await request(app).get(`/api/users/${otherTesterId}/profile`).set('Authorization', `Bearer ${testerToken}`);
