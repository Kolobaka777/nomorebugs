...
describe('is_onboarding — lead/admin-only', () => {
  it('a lead can create a course flagged is_onboarding', async () => {
    const res = await request(server).post('/api/custom-courses').set('Authorization', `Bearer ${leadToken}`).send(courseBody({ is_onboarding: true }));
  ...
  it('a proposing tester sending is_onboarding:true has it silently ignored', async () => {
    const res = await request(server).post('/api/custom-courses').set('Authorization', `Bearer ${testerToken}`).send(courseBody({ is_onboarding: true }));
    expect(res.status).toBe(200);
  ...
  it('a lead can toggle is_onboarding on an existing course via PUT', async () => {
    const created = await request(server).post('/api/custom-courses').set('Authorization', `Bearer ${leadToken}`).send(courseBody({ title: 'Toggle me' }));
    const id = created.body.id;
    expect(db.prepare('SELECT is_onboarding FROM custom_courses WHERE id = ?').get(id).is_onboarding).toBe(0);

    await request(server).put(`/api/custom-courses/${id}`).set('Authorization', `Bearer ${leadToken}`).send({ is_onboarding: true });
    expect(db.prepare('SELECT is_onboarding FROM custom_courses WHERE id = ?').get(id).is_onboarding).toBe(1);

    await request(server).put(`/api/custom-courses/${id}`).set('Authorization', `Bearer ${leadToken}`).send({ is_onboarding: false });
    expect(db.prepare('SELECT is_onboarding FROM custom_courses WHERE id = ?').get(id).is_onboarding).toBe(0);
  });
});
