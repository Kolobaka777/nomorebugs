...
describe('lecture video links — always a pasted URL, never a file upload', () => {
  it('GET /api/lectures/:id returns null video_url by default', async () => {
    const res = await request(server).get(`/api/lectures/${fixtures.lec1Id}`).set('Authorization', `Bearer ${testerToken}`);
    expect(res.status).toBe(200);
    expect(res.body.video_url).toBeNull();
  });

  it('GET /api/admin/lectures is lead-only and lists every lecture', async () => {
    const asTester = await request(server).get('/api/admin/lectures').set('Authorization', `Bearer ${testerToken}`);
    expect(asTester.status).toBe(403);

    const asLead = await request(server).get('/api/admin/lectures').set('Authorization', `Bearer ${leadToken}`);
  ...
  it('a lead can set a YouTube link with no warning', async () => {
    ...
    const check = await request(server).get(`/api/lectures/${fixtures.lec1Id}`).set('Authorization', `Bearer ${testerToken}`);
  ...
  it('a lead setting a link from an unrecognized host gets a soft warning, not a hard block', async () => {
    ...
    const check = await request(server).get(`/api/lectures/${fixtures.lec2Id}`).set('Authorization', `Bearer ${testerToken}`);
