...
describe('«Автор» — first ever approved proposal, any kind', () => {
  it('fires the first time a lead approves this tester\'s bug-example proposal', async () => {
    ...
    await request(app).patch(`/api/bug-examples/${created.body.id}/approve`).set('Authorization', `Bearer ${leadToken}`);
...
describe('«Библиотекарь» — 5 approved glossary terms from the same author', () => {
  it('does not fire on 4, fires on the 5th approval', async () => {
    for (let i = 0; i < 5; i++) {
      ...
      await request(app).patch(`/api/glossary/${created.body.id}/approve`).set('Authorization', `Bearer ${leadToken}`);
      if (i < 4) expect(hasBadge(fixtures.testerId, 'achievement_bibliotekar')).toBe(false);
    }
...
describe('«Наставник» — 3 approved guides from the same author', () => {
  it('fires on the 3rd approval, not before', async () => {
    for (let i = 0; i < 3; i++) {
      ...
      await request(app).patch(`/api/guides/${created.body.id}/approve`).set('Authorization', `Bearer ${leadToken}`);
    }
...
describe('«Голос команды» — 10+ likes across own suggestions, or 5 answered questions', () => {
  it('fires once cumulative likes across a tester\'s own posts reach 10', async () => {
    await request(app).post('/api/auth/register').send({ email: 'liked-author@test.local', password: 'likedpass123', name: 'Liked Author' });
    const likedAuthorToken = await loginAs(request, app, 'liked-author@test.local', 'likedpass123');
    const likedAuthor = db.prepare("SELECT id FROM users WHERE email = 'liked-author@test.local'").get();

    const post1 = await request(app).post('/api/suggestions').set('Authorization', `Bearer ${likedAuthorToken}`).send({ type: 'idea', text: 'Idea one', is_anonymous: false });
    const post2 = await request(app).post('/api/suggestions').set('Authorization', `Bearer ${likedAuthorToken}`).send({ type: 'idea', text: 'Idea two', is_anonymous: false });

    // 9 likes spread across both posts from distinct likers — not yet enough.
    for (let i = 0; i < 9; i++) {
      await request(app).post('/api/auth/register').send({ email: `liker-${i}@test.local`, password: 'likerpass123', name: `Liker ${i}` });
      const likerToken = await loginAs(request, app, `liker-${i}@test.local`, 'likerpass123');
      const targetId = i % 2 === 0 ? post1.body.id : post2.body.id;
      await request(app).post(`/api/suggestions/${targetId}/like`).set('Authorization', `Bearer ${likerToken}`);
    }
    expect(hasBadge(likedAuthor.id, 'achievement_golos_komandy')).toBe(false);

    // 10th like tips it over.
    await request(app).post('/api/auth/register').send({ email: 'liker-10@test.local', password: 'likerpass123', name: 'Liker 10' });
    const liker10Token = await loginAs(request, app, 'liker-10@test.local', 'likerpass123');
    await request(app).post(`/api/suggestions/${post1.body.id}/like`).set('Authorization', `Bearer ${liker10Token}`);
  ...
  it('also fires once a lead has answered 5 of a tester\'s questions', async () => {
    await request(app).post('/api/auth/register').send({ email: 'asker@test.local', password: 'askerpass123', name: 'Asker' });
    const askerToken = await loginAs(request, app, 'asker@test.local', 'askerpass123');
    const asker = db.prepare("SELECT id FROM users WHERE email = 'asker@test.local'").get();

    for (let i = 0; i < 5; i++) {
      const q = await request(app).post('/api/suggestions').set('Authorization', `Bearer ${askerToken}`).send({ type: 'question', text: `Question ${i}`, is_anonymous: false });
      if (i < 4) expect(hasBadge(asker.id, 'achievement_golos_komandy')).toBe(false);
      await request(app).patch(`/api/suggestions/${q.body.id}/answer`).set('Authorization', `Bearer ${leadToken}`).send({ answer: `Answer ${i}` });
    }
...
describe('«Полуночный жук» — logged in after midnight (server time) on 5 distinct days', () => {
  it('fires on the next login once 5 distinct early-hour login days already exist', async () => {
    await request(app).post('/api/auth/register').send({ email: 'nightowl@test.local', password: 'nightpass123', name: 'Night Owl' });
    ...
    await loginAs(request, app, 'nightowl@test.local', 'nightpass123');
  ...
  it('does not fire on only 4 distinct early-hour days', async () => {
    await request(app).post('/api/auth/register').send({ email: 'almostowl@test.local', password: 'almostpass123', name: 'Almost Owl' });
    ...
    await loginAs(request, app, 'almostowl@test.local', 'almostpass123');
...
describe('«Коллекционер» — all 5 skill-area badges crafted', () => {
  it('fires when the 5th and final skill badge is crafted', async () => {
    await request(app).post('/api/auth/register').send({ email: 'collector@test.local', password: 'collectorpass123', name: 'Collector' });
    const collectorToken = await loginAs(request, app, 'collector@test.local', 'collectorpass123');
