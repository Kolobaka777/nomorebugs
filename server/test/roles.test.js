...
describe('requireRole("lead") route protection', () => {
  it('rejects a tester with 403', async () => {
    const res = await request(server).get('/api/lead/team').set('Authorization', `Bearer ${testerToken}`);
  ...
  it('rejects an unauthenticated request with 401 before role is even checked', async () => {
    const res = await request(server).get('/api/lead/team');
    expect(res.status).toBe(401);
  });

  it('lets admin through too, without "admin" ever appearing in the route\'s allow-list', async () => {
    const res = await request(server).get('/api/lead/team').set('Authorization', `Bearer ${adminToken}`);
    expect(res.status).toBe(200);
  });
});
...
describe('requireRole("admin") route protection', () => {
  it('rejects a lead with 403 — being lead does not imply admin', async () => {
    const res = await request(server).get('/api/admin/users').set('Authorization', `Bearer ${leadToken}`);
    expect(res.status).toBe(403);
  });

  it('rejects a tester with 403', async () => {
    const res = await request(server).get('/api/admin/users').set('Authorization', `Bearer ${testerToken}`);
    expect(res.status).toBe(403);
  });
