...
describe('GET /api/lead/before-after-by-tester', () => {
  it('pairs a tester\'s baseline self-rating with their measured quiz performance in the same skill_area', async () => {
    ...
    const res = await request(server).get('/api/lead/before-after-by-tester').set('Authorization', `Bearer ${leadToken}`);
    expect(res.status).toBe(200);
  ...
  it('is lead-only', async () => {
    const res = await request(server).get('/api/lead/before-after-by-tester').set('Authorization', `Bearer ${testerToken}`);
    expect(res.status).toBe(403);
  });
});
